      ...
      if (evictionPolicy_->RemainingCapacity() <= evictionPolicy_->Capacity() / 3 * 2)
      {
        ...
        for (auto objId : objIds)
        {
          ...
          ret.push_back(putThreadPools[node]->enqueue([&, objId, node]() {
            ...
            entry->state = ObjectState::PLASMA_EVICTED;
            Put({objId}, {std::make_shared<arrow::Buffer>(entry->pointer, entry->data_size + entry->metadata_size)}, node);
            PlasmaAllocator::Free(entry->pointer, entry->data_size + entry->metadata_size);
            entry->pointer = nullptr;
            evictionPolicy_->RemoveObject(objId);
            entry->mtx.unlock();
                           ...
                           int numaId)
{
  ...
  for (int i = 0; i < total; i++)
  {
    ...
    if (ret != 0)
      ARROW_LOG(WARNING) << "vmemcache_put error:" << vmemcache_errormsg();
  }
  auto toc = std::chrono::steady_clock::now();
  std::chrono::duration<double> time_ = toc - tic;
                           ...
                           const std::vector<std::shared_ptr<Buffer>> &data)
{
  ...
  {
    if (results[i].get() != 0)
      ARROW_LOG(WARNING) << "Put " << i << " failed";
  }
  auto toc = std::chrono::steady_clock::now();
  std::chrono::duration<double> time_ = toc - tic;
  ARROW_LOG(DEBUG) << "Put " << total << " objects takes "
